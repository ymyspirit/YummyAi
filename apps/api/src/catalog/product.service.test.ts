...
import { createEntityId, type CreateSkuInput, type CreateSpuInput, type ProductPlanInput, type ProductStatus, type TenantContext } from "@yummyai/contracts";
...
describe("ProductService", () => {
  it("does not create an SPU from an unapproved product plan", async () => {
    ...
    await expect(service.createSpu(tenantA, plan.id, { code: "MUG", name: "Gift mug" })).rejects.toBeInstanceOf(InvalidTransitionError);
  });
  ...
  it("requires report evidence before approval", async () => {
    ...
    await expect(service.transition(tenantA, plan.id, "approved")).rejects.toBeInstanceOf(ApprovalEvidenceRequiredError);
  });
  ...
  it("rejects duplicate SKU codes inside one tenant but allows the same code in another tenant", async () => {
    ...
    await service.createSku(tenantA, { spuId: spuA.id, code: "mug-blk-11", attributes: {}, unitCost: { amount: 8.25, currency: "USD" } });
    await expect(service.createSku(tenantA, { spuId: spuA.id, code: "MUG-BLK-11", attributes: {} })).rejects.toBeInstanceOf(ConflictException);
    await expect(service.createSku(tenantB, { spuId: spuB.id, code: "MUG-BLK-11", attributes: {} })).resolves.toMatchObject({ code: "MUG-BLK-11" });
  });
  ...
  it("enforces explicit lifecycle transitions and preserves cost currency", async () => {
    ...
    await expect(service.transition(tenantA, plan.id, "ready")).rejects.toBeInstanceOf(InvalidTransitionError);
    ...
    expect([approved.status, approved.targetCost]).toEqual(["approved", { amount: 12.5, currency: "USD" }]);
  });
...
class MemoryCatalogRepository implements CatalogRepository {
  ...
  async createPlan(context: TenantContext, input: ProductPlanInput) {
    const plan: ProductPlanRecord = { id: createEntityId(), tenantId: context.tenantId, status: "researching", ...input };
    this.plans.push(plan);
    return plan;
  }
  async getPlan(context: TenantContext, id: string) { return this.plans.find((plan) => plan.tenantId === context.tenantId && plan.id === id); }
  async listPlans(context: TenantContext) { return this.plans.filter((plan) => plan.tenantId === context.tenantId); }
  ...
  async createSpu(context: TenantContext, plan: ProductPlanRecord, input: CreateSpuInput) {
    const spu: SpuRecord = { id: createEntityId(), tenantId: context.tenantId, productPlanId: plan.id, status: "developing", customization: plan.customization, ...input };
    this.spus.push(spu);
    return spu;
  }
  async getSpu(context: TenantContext, id: string) { return this.spus.find((spu) => spu.tenantId === context.tenantId && spu.id === id); }
  async findSkuByCode(context: TenantContext, code: string) { return this.skus.find((sku) => sku.tenantId === context.tenantId && sku.code === code); }
  async createSku(context: TenantContext, input: CreateSkuInput) {
    const sku: SkuRecord = { id: createEntityId(), tenantId: context.tenantId, status: "draft", ...input };
    this.skus.push(sku);
    return sku;
  }
}
