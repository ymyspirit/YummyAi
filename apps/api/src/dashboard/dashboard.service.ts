...
export interface DashboardRepository { getMetrics(context: TenantContext, range: UtcDateRange): Promise<Omit<DashboardMetrics, "range" | "risks"> & { failedJobs: number }>; }
...
export class DashboardService {
  ...
  async getMetrics(context: TenantContext, rawRange: DashboardDateRange): Promise<DashboardMetrics> {
    ...
    return { range, capture: metrics.capture, ai: metrics.ai, productFunnel: metrics.productFunnel, design: metrics.design, listing: metrics.listing, recentActivity: metrics.recentActivity, myTasks: metrics.myTasks, risks };
  }
}
...
export class DrizzleDashboardRepository implements DashboardRepository {
  ...
  async getMetrics(context: TenantContext, range: UtcDateRange) {
    const captures = await withTenant(this.database.db, context, (tx) => tx.select({ status: captureSnapshots.status, count: sql<number>`count(*)::int` }).from(captureSnapshots).where(and(gte(captureSnapshots.capturedAt, range.from), lt(captureSnapshots.capturedAt, range.toExclusive))).groupBy(captureSnapshots.status));
    const [cost] = await withTenant(this.database.db, context, (tx) => tx.select({ total: sql<string>`coalesce(sum(${aiBudgetLedger.amountUsd}), 0)` }).from(aiBudgetLedger).where(and(eq(aiBudgetLedger.state, "committed"), gte(aiBudgetLedger.createdAt, range.from), lt(aiBudgetLedger.createdAt, range.toExclusive))));
    ...
    const productRows = await withTenant(this.database.db, context, (tx) => tx.select({ status: productPlans.status, count: sql<number>`count(*)::int` }).from(productPlans).groupBy(productPlans.status));
    const activeDesigns = await withTenant(this.database.db, context, (tx) => tx.select({ id: designTasks.id, title: designTasks.title, status: designTasks.status, dueAt: designTasks.dueAt, createdBy: designTasks.createdBy }).from(designTasks).where(and(ne(designTasks.status, "approved"), ne(designTasks.status, "archived"))));
    const listingRows = await withTenant(this.database.db, context, (tx) => tx.select({ listingId: listings.id, versionId: listingVersions.id, completeness: sql<number>`coalesce((${listingVersions.validation}->>'completeness')::numeric, 0)`, blockerCount: sql<number>`jsonb_array_length(coalesce(${listingVersions.validation}->'blockers', '[]'::jsonb))` }).from(listings).innerJoin(listingVersions, eq(listingVersions.listingId, listings.id)).orderBy(desc(listingVersions.versionNumber)));
    ...
    return {
      capture,
      ai: { queued: latestJobs.filter((job) => job.state === "queued").length, running: latestJobs.filter((job) => job.state === "running").length, costUsd: Number(cost?.total ?? 0) },
