...
describe.sequential("customer intelligence", () => {
  ...
  it("pins VOC versions, isolates tenants, and keeps recommendations reviewable", async () => {
    ...
    const analysis = await service.calculateAnalysis(contextA, { definitionId: definition.id, expectedDefinitionVersion: 1, windowStart: "2026-07-01T00:00:00.000Z", windowEnd: "2026-08-01T00:00:00.000Z", evidenceCutoffAt: "2026-08-02T00:00:00.000Z", idempotencyKey: `ci-analysis-key-${tenantA}` });
    ...
    await expect(service.calculateAnalysis(contextB, { definitionId: definition.id, expectedDefinitionVersion: 1, windowStart: "2026-07-01T00:00:00.000Z", windowEnd: "2026-08-01T00:00:00.000Z", evidenceCutoffAt: "2026-08-02T00:00:00.000Z", idempotencyKey: `ci-cross-tenant-${tenantB}` })).rejects.toBeInstanceOf(Error);
