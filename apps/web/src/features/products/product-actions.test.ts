...
describe("createProductPlan", () => {
  ...
  it("uses lifecycle, SPU, and SKU API routes for product development", async () => {
    ...
    apiFetchMock.mockResolvedValue(
      new Response(JSON.stringify({ id: createEntityId() }), {
        headers: { "content-type": "application/json" },
        status: 201,
      }),
    );
    ...
    const spu = new FormData();
    spu.set("code", "pillow");
    spu.set("name", "Custom pillow");
    await createProductSpu(planId, idle, spu);
    const sku = new FormData();
    sku.set("code", "pillow-std");
    sku.set("attributes", "size: 16x16, color: natural");
    sku.set("unitCostAmount", "7.50");
    sku.set("unitCostCurrency", "USD");
    ...
    expect(JSON.parse(String(apiFetchMock.mock.calls[2]?.[1]?.body))).toMatchObject({
      attributes: { color: "natural", size: "16x16" },
      code: "PILLOW-STD",
      spuId,
    });
  });

  it("persists a validated customization schema through the product API", async () => {
    const planId = createEntityId();
    apiFetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify({ id: planId }), {
        headers: { "content-type": "application/json" },
        status: 200,
      }),
    );
    ...
    const formData = new FormData();
    formData.set("customization", JSON.stringify(customization));

    const result = await saveProductPlanCustomization(planId, idle, formData);

    expect(result).toMatchObject({ status: "success" });
    expect(apiFetchMock).toHaveBeenCalledWith(
      `http://api.test/v1/products/plans/${planId}/customization`,
      expect.objectContaining({
        method: "PATCH",
      }),
    );
    expect(JSON.parse(String(apiFetchMock.mock.calls[0]?.[1]?.body))).toEqual({ customization });
    expect(revalidatePathMock).toHaveBeenCalledWith("/products");
  });

  it("rejects an invalid customization schema before calling the API", async () => {
    const formData = new FormData();
    formData.set(
      "customization",
      JSON.stringify({
        version: 1,
        fields: [{ key: "Photo Upload", label: "", required: true, type: "short_text" }],
      }),
    );

    await expect(
      saveProductPlanCustomization(createEntityId(), idle, formData),
