...
async function loadAnalysis(reportId: string): Promise<{ report?: AnalysisReport; versions: AnalysisReport[]; error?: string }> {
  ...
  const headers: Record<string, string> = process.env.API_ACCESS_TOKEN
  ...
  try {
    ...
    if (!reportResponse.ok || !versionsResponse.ok) throw new Error(`分析报告读取失败 (${reportResponse.status}/${versionsResponse.status})`);
    return { report: await reportResponse.json() as AnalysisReport, versions: await versionsResponse.json() as AnalysisReport[] };
  } catch (error) {
...
function demoVersions(reportSeriesId: string): AnalysisReport[] {
  const series = /^[0-9a-f-]{36}$/i.test(reportSeriesId) ? reportSeriesId : "0198fbef-4a10-7000-8000-000000000021";
  ...
  const first: AnalysisReport = {
    ...
    sections: [{ id: "market", title: "市场信号", claims: [{ id: "price-band", kind: "fact", text: "两个公开价格分别为 29.99 美元与 42.00 美元。", evidence: [{ snapshotId: snapshotA, sourceType: "field", sourcePath: "price.amount", excerpt: "$29.99" }, { snapshotId: snapshotB, sourceType: "field", sourcePath: "price.amount", excerpt: "$42.00" }] }] }],
  };
  return [first, {
    ...common,
    ...
    executiveSummary: "证据显示中价位商品以交付速度取胜，高价商品依赖个性化与礼盒体验；新方案应优先验证轻定制组合。",
    sections: [
      { id: "market", title: "市场与价格", summary: "公开页面证据显示两个清晰的价值层级。", claims: [
        { id: "price-band", kind: "fact", text: "两个公开价格分别为 29.99 美元与 42.00 美元，价差约 40%。", evidence: [{ snapshotId: snapshotA, sourceType: "field", sourcePath: "price.amount", excerpt: "$29.99" }, { snapshotId: snapshotB, sourceType: "field", sourcePath: "price.amount", excerpt: "$42.00" }] },
        { id: "premium-signal", kind: "inference", text: "高价商品的溢价主要来自刻字选项和礼盒呈现，而不是基础材质差异。", confidence: 0.78, evidence: [{ snapshotId: snapshotB, sourceType: "field", sourcePath: "content.personalization", excerpt: "Add a name and gift message" }] },
      ] },
      { id: "opportunity", title: "行动机会", claims: [{ id: "bundle-test", kind: "recommendation", text: "先测试可选刻字与标准礼盒的轻定制组合，并把交付时效作为首屏证据。", priority: "high", evidence: [{ snapshotId: snapshotA, sourceType: "field", sourcePath: "shipping.promise", excerpt: "Ships within 24 hours" }] }] },
    ],
    comparison: [
      { dimension: "公开价格", values: { [snapshotA]: "$29.99", [snapshotB]: "$42.00" }, evidence: [{ snapshotId: snapshotA, sourceType: "field", sourcePath: "price.amount" }, { snapshotId: snapshotB, sourceType: "field", sourcePath: "price.amount" }] },
      { dimension: "定制能力", values: { [snapshotA]: "固定款式", [snapshotB]: "姓名刻字 + 赠言" }, evidence: [{ snapshotId: snapshotB, sourceType: "field", sourcePath: "content.personalization" }] },
      { dimension: "交付承诺", values: { [snapshotA]: "24 小时发货", [snapshotB]: "3–5 个工作日" }, evidence: [{ snapshotId: snapshotA, sourceType: "field", sourcePath: "shipping.promise" }] },
    ],
  }];
}
