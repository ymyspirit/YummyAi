...
Bulk work is an immutable `marketplace_publication_batches` aggregate that pins one tenant, account, capability snapshot, marketplace, schedule, parent batch, and original 2–100 item order. Each item remains an ordinary immutable publication request linked by `batch_id`, so the existing validation, rights, event, status, and tenant boundaries stay authoritative. Initial Amazon items run validation previews and initial Etsy items create drafts. Continuation creates a second immutable batch only after every parent item reaches its platform-specific success state.

One Amazon continuation batch produces exactly one JSON Listings Feed. The Worker creates an upload document, uploads the JSON payload, creates the Feed, polls its processing status, downloads the processing report, and maps issues back to child requests by the stable one-based `messageId` from the original batch order. Terminal items may be omitted from a retry payload without renumbering later items. Unknown message IDs, an incomplete processed-item count, an unknown Feed-creation outcome, or failed Feed-ID persistence enters `reconciliation_required`; the Worker never falls back to per-item submission or blindly creates another Feed. Accepted items enter the identifier-only publication reconciliation queue for the normal online status check. Etsy continuation keeps one activation job per child request and reuses the existing step-level recovery path.
...
Online Listing reconciliation uses separate immutable `marketplace_listing_sync_requests` and append-only `marketplace_listing_sync_events`. A request pins the approved Listing version and the published request that owns the account, marketplace, and external Listing ID. Narrow actions compare only price/inventory; explicit full-content actions additionally compare the supported writable text, category/configuration, and personalization projection. Amazon full replacement uses the approved product type and attributes. Etsy full replacement is a guarded sequence of Listing, inventory, and personalization writes, so a later-step failure is treated as an uncertain partial outcome. Provider-generated IDs and Money wrappers are not business drift, and media remains in the separate immutable publication/upload workflow.
...
17. Every online push uses only the current approved Listing version and compares normalized provider state by a deterministic checksum scoped to the explicit narrow or full-content action.
...
25. Publication batches and their parent/item order are immutable; one normalized account/marketplace/item set produces at most one initial batch, so a retry cannot change the first accepted order or schedule. One parent batch produces at most one continuation batch.
26. One Amazon continuation batch creates at most one JSON Listings Feed, and report items retain their original stable `messageId` mapping even when terminal requests are filtered from later processing.
27. A Feed response that cannot be completely mapped or whose creation/writeback outcome is unknown requires manual reconciliation and cannot be automatically resubmitted.
