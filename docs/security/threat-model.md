...
| A forecast silently replaces operational facts | pinned input window/evidence/model/quantiles, append-only evaluations and overrides, no mutation dependency from planning into inventory/procurement/Listing/finance | planning contract, integration, and privilege tests |
| Stale or incomplete cockpit data appears current | versioned definitions, observed time, maximum age, completeness threshold, explicit state, drill-through evidence, and reconciliation queue | metric snapshot and projection-rebuild integration tests |
| API client selects another tenant or escalates scopes | tenant resolved from exact token digest, restricted database authentication function, creator-scope subset, expiry/revocation, no tenant header | guard plus wrong-secret/expiry/revocation/cross-tenant integration tests |
| API token or Webhook signing secret is disclosed | one-time plaintext response, dedicated encrypted secret domain, digest-only API token storage, no secret in workspace/audit/job/log | contract, vault, workspace, redaction, and integration tests |
| Webhook target is used for SSRF or insecure egress | HTTPS-only endpoint policy with loopback development exception, deployment DNS/IP egress allowlist, bounded timeout | contract tests and deployment review |
